...
export class BuyButton extends GameBoardComponent {
  ...
  onClick = () => {
    if (
      ...
      this.app.startAssetAcquisitionRequest(this.panel.assetId)
    }
  }
  ...
  private drawText = () => {
    ...
    if (this.panel?.type === 'asset') {
      this.buttonText = new PIXI.Text('買収する', buttonTextStyle)
      this.buttonText.x = 5
      this.buttonText.y = 5
