...
export class ReleaseMortgageButton extends GameBoardComponent {
  ...
  onClick = () => {
    if (
      ...
      window.confirm(
        `${assetList[this.panel.assetId].name}の抵当を解除しますか？`
      )
    ) {
      this.app.releaseMortgageAssetRequest(this.panel.assetId)
    }
  }
  ...
  private drawBackground = () => {
    ...
    if (this.panel?.type === 'asset') {
      const backgroundColor = this.poolModel?.isMyTurn ? 0xe91e63 : 0xc4c4c4
  ...
  private drawText = () => {
    ...
    if (this.panel?.type === 'asset') {
      this.buttonText = new PIXI.Text('抵当を解除', buttonTextStyle)
      this.buttonText.x = 5
      this.buttonText.y = 5
